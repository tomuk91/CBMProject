                ...
                <div class="flex-1 space-y-4">
                    ...
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl rounded-2xl border border-gray-100 dark:border-gray-700">
                            ...
                            <div class="flex items-center justify-between flex-wrap gap-4">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-red-100 mt-1 sm:mt-2 text-sm sm:text-base md:text-lg">{{ __('messages.vehicle_manage_description') }}</p>
                                    </div>
                                </div>
                                <button onclick="openAddVehicleModal()" 
                                        class="px-4 py-2 sm:px-6 sm:py-3 bg-white hover:bg-gray-50 text-red-600 font-semibold rounded-xl transition-all duration-200 shadow-lg hover:shadow-xl flex items-center text-sm sm:text-base">
                        ...
                        <div class="grid grid-cols-3 divide-x divide-gray-200 dark:divide-gray-700">
                            <div class="px-3 py-3 sm:px-6 sm:py-4 text-center">
                                <div class="text-xl sm:text-2xl md:text-3xl font-bold text-red-600 dark:text-red-400">{{ $vehicles->count() }}</div>
                                <div class="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mt-1">{{ __('messages.total_vehicles') }}</div>
                            </div>
                            <div class="px-3 py-3 sm:px-6 sm:py-4 text-center">
                                <div class="text-xl sm:text-2xl md:text-3xl font-bold text-green-600 dark:text-green-400">{{ $vehicles->where('is_primary', true)->count() }}</div>
                                <div class="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mt-1">{{ __('messages.primary_vehicle') }}</div>
                            </div>
                            <div class="px-3 py-3 sm:px-6 sm:py-4 text-center">
                                <div class="text-xl sm:text-2xl md:text-3xl font-bold text-blue-600 dark:text-blue-400">{{ $vehicles->pluck('make')->unique()->count() }}</div>
                                <div class="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mt-1">{{ __('messages.different_makes') }}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start">
                        ...
                        <div>
                            <p class="text-blue-800 dark:text-blue-200 font-medium text-sm">
                                {{ __('messages.vehicle_limit_info', ['count' => $vehicles->count(), 'max' => 4, 'remaining' => 4 - $vehicles->count()]) }}
                            </p>
                        </div>
                    </div>
                </div>
